    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                break;
            case MotionEvent.ACTION_MOVE:
                float direction = interceptDownY - ev.getY();
                boolean canScrollVertically = canScrollVertically((int) (direction));
                int[] location = new int[2];
                zoomView.getLocationOnScreen(location);
                return (direction < 0 && location[1] == 0 && !canScrollVertically) || (direction > 0 && canScrollVertically);
        }
